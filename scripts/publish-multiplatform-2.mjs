...
const SLIDES_MP4 = path.join(__dirname, '../projetos/carrossel-remotion/out/slides2');
const SLIDES_PNG = path.join(__dirname, '../projetos/carrossel-remotion/out/png2');

const LEGENDA = `Instalei o Gemma 4 no iPhone hoje. Rodou offline.

Open source, Apache 2.0. Sem royalty em produto comercial. A base é a mesma do Gemini 3, de graça.

Antes de eu perguntar qualquer coisa, ele me mandou um guia de como tirar mais do modelo. Fórmula: objetivo + contexto + formato = resultado.

O Google não fez isso por bondade. Mas enquanto todo mundo discute se vale a pena, quem já está usando constrói.
