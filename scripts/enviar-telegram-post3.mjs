...
const SLIDES_DIR = '/home/user/dobralabs-ccos-ratos/projetos/carrossel-remotion/out/post3';
...
Tema: Claude Code OS: o que muda pro empresário que não sabe programar nada
Ângulo: Mostra 3 automações reais que qualquer PME pode rodar hoje sem dev: extrator de relatório do Meta Ads → planilha, resposta automática de leads via WhatsApp com contexto de campanha, gerador de brief de criativo a partir de histórico de vendas. Foco em resultado, zero em conceito técnico.
Fonte: Stage Mídia / Anthropic Claude — fluxos testados em produção
...
try {
  ...
  legenda = 'O Claude Code OS chegou — e o empresário que não sabe programar nada pode rodar automações reais hoje. 3 fluxos testados em produção no carrossel. #claudecode #ia #automatizacao #marketingdigital #stagemidia';
...
await sendMessage(`📝 LEGENDA PRONTA (Post 18h):\n\n${legenda}`);
